...
#### Description: Test models with Venezuelan refugees / displaced abroad
...
demref2020.ori.asy.age <- demref2020.ori.asy.age %>% 
  ...
  select(origin, origin_iso3, origin_country, asylum, asylum_iso3, asylum_country, `asylum_Region Name`,
         children, totalEndYear, typeOfDisaggregationAge) %>% # add-hoc adding land neighbours for Venezuela as covariate
  mutate(neighbour = case_when(
    asylum_iso3 %in% c("COL", "BRA", "GUY") ~ 1,
    !(asylum_iso3 %in% c("COL", "BRA", "GUY")) ~ 0
    )
  ) %>% 
...
saveRDS(m.child.1, file = "output/mchild1-ven.rds")
...
saveRDS(m.child.2, file = "output/mchild2-ven.rds")
...
saveRDS(m.child.3, file = "output/mchild3-ven.rds")
